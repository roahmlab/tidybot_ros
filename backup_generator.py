...
class Kinova(tfds.core.GeneratorBasedBuilder):
    ...
    def _generate_examples(self, parquet_files: list) -> Iterator[Tuple[str, Any]]:
        ...
        for pq_file in parquet_files:
            ...
            # Load corresponding mp4 paths
            exterior_mp4_path = os.path.join(EXTERN_CAMERA_DIR, f"{episode_id}.mp4")
            wrist_mp4_path = os.path.join(WRIST_CAMERA_DIR, f"{episode_id}.mp4") 
            ...
            for i, row in data.iterrows():
                step = {
                    'observation': {
                        # Video paths (strings) to mp4s (or relative paths)
                        'image': exterior_mp4_path,
                        'wrist_image': wrist_mp4_path,
                        'state': row['observation.state'].astype(np.float32),
                        'state_ros': row['observation.state_ros'].astype(np.float32),
                        'images': {
                            'cam_exterior': exterior_mp4_path,
                            'cam_wrist': wrist_mp4_path,
                        }
                    },
