            ...
            match msg.teleop_mode:
                case "base":
                    if self.base_xr_ref_pos is None:
                        self.base_xr_ref_pos = np.array(
                            [msg.pos_x, msg.pos_y, msg.pos_z]
                        )
                        self.base_xr_ref_quat = np.array(
                            [msg.or_x, msg.or_y, msg.or_z, msg.or_w]
                        )
                        self.base_ref_pos = np.array(
                            [self.base_obs[1], self.base_obs[0], 0.0]
                        )
                        ...
                        delta_quat = quaternion_multiply(
                            quaternion_inverse(self.base_xr_ref_quat), xr_quat
                        )
                        ...
                        yaw = math.atan2(
                            2
                            * (
                                delta_quat[3] * delta_quat[2]
                                + delta_quat[0] * delta_quat[1]
                            ),
                            delta_quat[3] ** 2
                            - delta_quat[2] ** 2
                            - delta_quat[1] ** 2
