    ...
    def _generate_examples(self, parquet_files: list) -> Iterator[Tuple[str, Any]]:
        ...
        for pq_file in parquet_files:
            ...
            for i, frame in enumerate(exterior_container.decode(video=0)):
                exterior_frames[i] = frame.to_ndarray(format='bgr24')

            # Build frame index to frame dict for wrist camera
            wrist_frames = {}
            for i, frame in enumerate(wrist_container.decode(video=0)):
                wrist_frames[i] = frame.to_ndarray(format='bgr24')
            ...
            for i, row in data.iterrows():
                ...
                ext_frame = exterior_frames.get(frame_idx)
                wrist_frame = wrist_frames.get(frame_idx)

                if ext_frame is None or wrist_frame is None:
                    raise RuntimeError(f"Failed to read frame {frame_idx} for episode {episode_id}")

                # Convert from BGR to RGB
                ext_frame_rgb = cv2.cvtColor(ext_frame, cv2.COLOR_BGR2RGB)
                ...
                step = {
                    'observation': {
                        ...
                        'images': {
                            'cam_exterior': ext_frame_rgb,
                            'cam_wrist': wrist_frame_rgb,
                        }
                    },
